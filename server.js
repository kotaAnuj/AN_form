 ...
 * Firebase authentication/token verification, logging, and Swagger documentation.
 * 
 * Required packages:
...
/**
 * Logs form creation details into Firebase Realtime Database.
 */
...
app.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      // Log form creation to Firebase DB
...
app.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      // Log form and sheet creation to Firebase DB
...
app.get('/api/forms', verifyFirebaseToken, async (req, res, next) => {
  try {
    const db = admin.database();
    const ref = db.ref('forms');
    const snapshot = await ref.orderByChild('userUid').equalTo(req.user.uid).once('value');
